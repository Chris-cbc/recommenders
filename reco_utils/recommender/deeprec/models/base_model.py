...
class BaseModel:
    ...
    def _get_initializer(self):
        if self.hparams.init_method == "tnormal":
            ...
            return tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=(
                "uniform" if False else "truncated_normal"), seed=self.seed)
        elif self.hparams.init_method == "xavier_uniform":
            return tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=(
                "uniform" if True else "truncated_normal"), seed=self.seed)
