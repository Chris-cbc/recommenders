...
class AnnealingCallback(Callback):
    ...
    def on_batch_end (self, epoch, logs={}):
        ...
        new_beta = min(1. * self.update_count / self.total_anneal_steps, self.anneal_cap)
...
class Mult_VAE:
    ...
    def _create_model(self):
        ...
        self.h = Dense(self.intermediate_dim, 
                       activation='tanh', 
                       kernel_initializer=keras.initializers.glorot_uniform(seed=self.seed), 
                       bias_initializer=keras.initializers.truncated_normal(stddev=0.001, seed=self.seed)
    ...
    def _take_sample(self, args): 
        ...
        # epsilon 
        epsilon = K.random_normal(shape=(K.shape(_mean)[0], self.latent_dim),
                                  mean=0.0, 
                                  stddev = 1.0,
                                  seed=self.seed
                                  )
    ...
    def nn_batch_generator(self, x_train):
        ...
        Args: 
            x_train (np.array, float): the click matrix for the train set.
        """
        # Shuffle the batch
        np.random.seed(self.seed) 
    ...
    def fit(self, x_train, x_valid, x_val_tr, x_val_te, mapper):
        ...
        Args: 
            x_train (np.array, float): the click matrix for the train set.
            x_valid (np.array, float): the click matrix for the validation set.
            x_val_tr (np.array, float): the click matrix for the validation set training part.
            x_val_te (np.array, float): the click matrix for the validation set testing part.
            mapper (AffinityMatrix): the mapper for converting click matrix to dataframe.
        """
